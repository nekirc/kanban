...
export async function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith('/dashboard') || pathname.startsWith('/board')) {
    ...
    // If 2FA is NOT verified, redirect to 2FA page (always require setup/verification)
    const is2FAVerified = request.cookies.get('zf_2fa_verified')?.value === 'true';

    if (!is2FAVerified && !pathname.startsWith('/auth/2fa')) {
      return NextResponse.redirect(new URL('/auth/2fa', request.url));
    }
  }

  // If already logged in and 2FA verified, redirect away from login/register
